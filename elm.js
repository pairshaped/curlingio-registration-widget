...
var author$project$Main$init = function (flags) {
	return _Utils_Tuple2(
		{host: flags.host, items: author$project$Types$Loading, section: flags.section},
		A2(author$project$Main$getItems, flags.host, flags.section));
};
		...
		if (result.$ === 'Ok') {
			...
			return _Utils_Tuple2(
				_Utils_update(
					model,
					{
						items: author$project$Types$Success(items)
					}),
				...
				switch (err.$) {
					...
					case 'BadStatus':
						...
						return A2(
							...
							model.section,
							_List_fromArray(
								['leagues', 'competitions', 'products'])) ? 'Invalid response status from server' : ('The section paramter passed is incorrect. \"' + (model.section + '\" is not a valid section.'));
					default:
						var string = err.a;
			...
			return _Utils_Tuple2(
				_Utils_update(
					model,
					{
						items: author$project$Types$Failure(errorMessage)
					}),
				elm$core$Platform$Cmd$none);
		}
	});
...
var author$project$View$view = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				author$project$View$viewItems(model.items)
			]));
};
